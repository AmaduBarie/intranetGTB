...
console.log(req.body.url)
  ...
  .post('/profile', (req, res) => {
    ...
    calenders.deleteOne({_id:req.body._id}, (err, doc) =>{ 
      console.log(doc)
      console.log(err)
      if(doc) return res.status(200).end('success');
  ...
  .post('/login', (req, res) => {
    ...
    console.log(req.signedCookies.XYZ)
    try {
      if(req.signedCookies.XYZ){

      }
      users.findOne({ $and: [{ $or: [{ email: req.body.email }, { username: req.body.email }] }, { password: req.body.password }] }, async (err, doc) => {
        ...
        if (doc) {
          ...
          users.find({ $and: [{ department: doc.department }, { _id: { $ne: doc._id } }] })
            ...
            .exec(function (err, atten) {
              ...
              if (alldata.event && alldata.chats) {
                ...
                res.cookie('XYZ', doc._id, { sameSite: 'lax', signed: true, expires: new Date(Date.now() + (60 * 60 * 24 * 30)), httpOnly: true }).status(200).send({ ...alldata, me: { department: doc.department, name: doc.username, img: doc.img, i: doc._id }, atten })
              } else {
          ...
          calenders.find({ $or: [{ setter: doc._id }, { "attendance._id": `${doc._id}` }] }, (err, event) => {
            if (alldata.me && alldata.chats) {
              // .cookie('XYZ', doc._id, {domain:"http://localhost:5000/" ,sameSite:'none', signed: true, expires: new Date(Date.now() + ( 60 * 60 * 24 * 30)), httpOnly: true })
              res.cookie('XYZ', doc._id, { sameSite: 'lax', signed: true, expires: new Date(Date.now() + (60 * 60 * 24 * 30)), httpOnly: true }).status(200).send({ ...alldata, event })
            } else {
          ...
          notifications.find({ $or: [{ setter: doc._id }, { "attendance._id": `${doc._id}` }] }, (err, chats) => {
            if (alldata.me && alldata.event) {
              // .cookie('XYZ', doc._id, {domain:"http://localhost:5000/" ,sameSite:'none', signed: true, expires: new Date(Date.now() + ( 60 * 60 * 24 * 30)), httpOnly: true })
              res.cookie('XYZ', doc._id, {sameSite: 'lax', signed: true, expires: new Date(Date.now() + (60 * 60 * 24 * 30)), httpOnly: true }).status(200).send({ ...alldata, chats })
            } else {
          ...
          res.status(200).cookie('x','masie',{sameSite:'lax'}).send('Incorrect details')
        };
      })
    } catch (error) {
      console.log(error)
